...
export function launchCore({ dataDir, forjaRoot, project, spawnRunner = defaultSpawnRunner } = {}) {
  const state = coreObservation(project.path);
  if (!state?.run || state.run.status !== 'running' || state.runnerAlive || (project.runId && state.run.run_id !== project.runId)) return undefined;
