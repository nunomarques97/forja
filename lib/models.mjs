...
//
// The Sponsor picks how much model power a run may spend: `max` (default),
// `high` or `eco`. The forjalvl lives in the run's state (`RUN.json.forjalvl`,
...
// Empty/missing means "the default forjalvl" (max); anything else unknown
// throws, so a typo on the CLI or in SETTINGS.json is refused instead of
// silently running the whole run on the wrong forjalvl.
export function normalizeLevel(x) {
  if (x === undefined || x === null || x === '') return 'max';
...
export const levelOf = value => { try { return normalizeLevel(value); } catch { return 'max'; } };
