...
//
// Why it exists (incident of 17 set 2026): the runner of the juniper-hill project
...
import { launchRunner, launchForja } from './spawn-runner.mjs';
...
export function guardPlan(list, state, now) {
  ...
  for (const p of Array.isArray(list) ? list : []) {
    ...
    const driver = run.driver === 'runner' || run.driver === 'interactive' ? run.driver : 'unknown';
    if (driver !== 'runner') {
    ...
    } else actions.push({ name, path: p.path, visible: run.visible === true, attempt: tries + 1 });
    next.projects[name] = writeEntry(e);
  }
...
export async function runGuardOnce({
  ...
  for (const a of plan.actions) {
    ...
    try {
      pid = spawn({
        dataDir: dir, forjaRoot: forja, project: { name: a.name, path: a.path },
    ...
    await safeNotify(notify, relaunchMessage(a.name, a.attempt, GUARD_MAX_ATTEMPTS), withClick({ priority: 'low', tags: ['arrows_counterclockwise'], dedup: false }, click));
  }
