...
// do Juniper Hill morreu e nada o relançou, porque o único vigia — o watchdog
...
export async function runPeerCheckOnce({
  ...
  if (plan.act === 'relaunch') {
    ...
    if (pid) {
      ...
      await safeNotify(notify, peerRelaunchMessage(peer, attempt, PEER_MAX_ATTEMPTS), withClick({ priority: 'low', tags: ['arrows_counterclockwise'], dedup: false }, click));
    } else {
