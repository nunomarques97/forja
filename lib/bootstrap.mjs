// `forja bootstrap <repo> [--dry-run]` — installs Forja into another repo,
...
export function managedBlock(forja = forjaRoot) {
  return [
    ...
    '## Forja',
    `- Forja repo: \`${forja}\``,
    // Preparing the project (this bootstrap) is one action; choosing who executes
    // a run is another, made per run (docs/ARCHITECTURE.md §3c). The block names
    // both ways as equals and never implies that a run must go to the runner.
    ...
    `  - In a conversation (the Claude Code extension or a terminal session is the Lead): load skill \`forja-lead\` and run \`node "${fwd(forja)}/bin/forja.mjs" run start --goal "<goal>"\` — the run is \`interactive\`, and the guard never launches a runner on it.`,
    `  - Unattended: \`node "${fwd(forja)}/bin/forja.mjs" runner --goal "<goal>"\` from this folder (see \`${join(forja, 'docs', 'RUNBOOK-UNATTENDED.md')}\`) — the run is \`runner\`, and the guard recovers it if the runner dies.`,
    `  - Hand-over between the two is explicit: \`node "${fwd(forja)}/bin/forja.mjs" run driver show|set interactive|runner\`.`,
...
export async function bootstrap({ pos = [], opt = {} } = {}) {
  ...
  if (!dryRun) {
    try {
      const entry = upsertProject({ path: plan.target });
