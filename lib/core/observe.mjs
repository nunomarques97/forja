...
import { validateTechnologyState, pendingTechnology } from './technology.mjs';
import { sanitizeClick } from '../notify.mjs';
import { recoveryInfo } from './recovery.mjs';
...
const records = (v) => (Array.isArray(v) ? v : []);
// Latest validation counts only; commands, logs and output never leave the process.
function validationSummary(tasks) {
  const summary = { passed: 0, failed: 0, unknown: 0, tasks_without_records: 0 };
  for (const t of tasks) {
    const validation = records(t?.validation);
    if (!validation.length) summary.tasks_without_records++;
    for (const v of validation)
      summary[v?.passed === true ? 'passed' : v?.passed === false ? 'failed' : 'unknown']++;
  }
  return summary;
}
...
export function coreObservation(root, { details = false, alive } = {}) {
  try {
    ...
    result.recovery = recoveryInfo(r, { alive: result.runnerAlive });
    validateTechnologyState(r);
    result.technology = pendingTechnology(r).map(d => ({
      id: d.id, capability: d.capability, constraints: d.constraints, recommended: d.recommended, rationale: d.rationale,
      options: d.options.map(o => ({ id: o.id, name: o.name, cost: o.cost, cost_basis: o.cost_basis, tradeoffs: o.tradeoffs,
        sources: o.evidence.map(e => sanitizeClick(e)).filter(Boolean),
      })),
    }));
    result.sponsor_notice = r.sponsorNotice ? { delivered: r.sponsorNotice.delivered === true, skipped: str(r.sponsorNotice.skipped, 80) } : null;
    ...
    const safeRows = rows.map((x) => ({
      ...
      time_limit_ms: measured(x.time_limit_ms),
      context_limit_tokens: measured(x.context_limit_tokens),
      context_limit_reached: x.context_limit_reached === true,
      timed_out: x.timed_out === true,
      rate_limited: x.rate_limited === true,
    ...
    result.pending = p
      ...
      : null;
    const stop = readStopRequest(root, r.run_id);
    result.stop_request = stop ? { mode: stop.mode, task: stop.task, requested_at: stop.requested_at } : null;
    result.tasks = r.tasks.map((t) => ({
      ...
      checks_total: Math.max(records(t.checks).length, records(t.validation).length),
      checks_passed: records(t.validation).filter((v) => v?.passed === true).length,
      review: str(t.review?.status, 32),
    }));
    result.validation_summary = validationSummary(r.tasks);
    return result;
  } catch {
    return {
      run: null,
      runnerAlive: true,
      error: 'Core state is unreadable; check locally before recovering.',
    };
  }
}
