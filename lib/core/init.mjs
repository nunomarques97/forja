...
function legacyRunActive(root) {
  ...
  try {
    return ['running', 'blocked'].includes(JSON.parse(readFileSync(path, 'utf8')).status);
  } catch {
    return true;
  }
}
...
function legacyAgents(root) {
  ...
  for (const agent of LEGACY_AGENTS) {
    ...
    if (/forja/i.test(before.toString('utf8'))) found.push({ agent, name, path, before });
  }
...
export function initCore(root, { dryRun = false } = {}) {
  ...
  try {
    ...
    for (const { source, target, archived } of agents.moves) {
      if (!archived) {
        journal.push(target);
        writeFileSync(target.path, source.before, { flag: 'wx' });
      }
      journal.push(source);
