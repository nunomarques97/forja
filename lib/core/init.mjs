...
import { CORE_METHOD_SKILLS, LEGACY_AGENTS, LEGACY_AGENT_ARCHIVE, LEGACY_SKILLS, disableLegacySkill, migrateInstructions } from './instructions.mjs';
...
function preflight(root) {
  ...
  for (const skill of [...LEGACY_SKILLS, ...CORE_METHOD_SKILLS]) {
  ...
  return { outputs, state: inside(root, '.forja'), stateExists: Boolean(stat), agents: legacyAgents(root) };
}
// An active legacy run may still delegate to its crew: unknown counts as active.
function legacyRunActive(root) {
  const path = inside(root, 'docs/forja/RUN.json');
  if (!existsSync(path)) return false;
  try {
    const state = JSON.parse(readFileSync(path, 'utf8'));
    return !state || Array.isArray(state) || !['finished', 'failed', 'abandoned'].includes(state.status);
  } catch {
    return true;
  }
}
// Crew agents stay selectable by description while they sit in .claude/agents.
// They are moved, never deleted, to an archive the legacy workflow can restore.
function legacyAgents(root) {
  const found = [];
  for (const agent of LEGACY_AGENTS) {
    const name = `.claude/agents/${agent}.md`;
    const path = inside(root, name);
    const stat = lstatSync(path, { throwIfNoEntry: false });
    if (!stat) continue;
    if (!stat.isFile() || stat.isSymbolicLink())
    ...
    // A user's own agent with a crew name is not ours to move.
    const header = before.toString('utf8').match(/^---\r?\n([\s\S]*?)\r?\n---/i)?.[1];
    if (header && new RegExp(`^name: *${agent}\\s*$`, 'm').test(header) && /\bforja(?:-| )crew\b/i.test(header))
      found.push({ agent, name, path, before });
  }
  if (!found.length) return { moves: [], directories: [], kept: [] };
  if (legacyRunActive(root))
    return { moves: [], directories: [], kept: found.map((f) => f.name), reason: 'active legacy run' };
  const directories = [];
  let relative = '';
  for (const part of LEGACY_AGENT_ARCHIVE.split('/')) {
    relative = relative ? `${relative}/${part}` : part;
    const path = inside(root, relative);
    const stat = lstatSync(path, { throwIfNoEntry: false });
    if (!stat) directories.push({ name: relative, path, directory: true });
    else if (stat.isSymbolicLink() || !stat.isDirectory())
      throw new Error(`${relative} must be a real directory, found ${kind(stat)}.`);
  }
  const moves = found.map((source) => {
    const name = `${LEGACY_AGENT_ARCHIVE}/${source.agent}.md`;
    const path = inside(root, name);
    const stat = lstatSync(path, { throwIfNoEntry: false });
    if (stat && (!stat.isFile() || stat.isSymbolicLink() || !readFileSync(path).equals(source.before)))
      throw new Error(`${name} already exists with different content; reconcile it with ${source.name} by hand. Nothing was written.`);
    return { source, target: { name, path, before: null }, archived: Boolean(stat) };
  });
  return { moves, directories, kept: [] };
}
...
// dryRun inspects and prepares everything, then reports the changes unwritten.
export function initCore(root, { dryRun = false } = {}) {
  root = resolve(root);
  if (!existsSync(root)) throw new Error('Project directory does not exist.');
  const { outputs, state, stateExists, agents } = preflight(root);
  const operations = [];
  ...
  const report = {
    ...
    legacy_agents: {
      archived: agents.moves.map(({ source, target }) => ({ from: source.name, to: target.name })),
      kept: agents.kept,
      ...(agents.reason ? { reason: agents.reason } : {}),
    },
  ...
  if (dryRun)
    return {
      ...
      dryRun: true,
      changes: [
        ...operations.filter(([output, content]) => !output.before?.equals(Buffer.from(content, 'utf8'))).map(([output]) => output.name),
        ...(stateExists ? [] : ['.forja']),
      ],
    };
  const journal = [];
  try {
    ...
    for (const directory of agents.moves.length ? agents.directories : []) {
      journal.push(directory);
      try { mkdirSync(directory.path); }
      catch (error) {
        if (error?.code === 'EEXIST') journal.pop();
        throw error;
      }
    }
    for (const { source, target, archived } of agents.moves) {
      if (!archived) {
        journal.push(target);
        try { writeFileSync(target.path, source.before, { flag: 'wx' }); }
        catch (error) {
          // A file created after preflight belongs to another writer.
          if (error?.code === 'EEXIST') journal.pop();
          throw error;
        }
      }
