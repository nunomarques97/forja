...
// Crew agent definitions of the legacy workflow, including the older names.
export const LEGACY_AGENTS = [
  'architect', 'product-manager', 'product-designer', 'technology-scout',
  'backend-dev', 'frontend-dev', 'reviewer', 'qa', 'security-reviewer',
  'ferreiro', 'bigorna', 'tracador', 'fundidor', 'lapidador', 'contraste',
];
export const LEGACY_AGENT_ARCHIVE = 'docs/forja/legacy-agents';
// Core workers read frozen copies of these methods through specialist_context;
// a project copy must never be loaded by a conversation on its own.
export const CORE_METHOD_SKILLS = ['planner', 'design', 'frontend', 'backend', 'reviewer', 'security'].map((id) => `forja-core-${id}`);
...
const PLAIN_INVALID = /^[-?:,[\]{}#&*!|>'"%@`]|:\s|:$|\s#/;
function scalarProblem(value) {
  if (value === '') return null;
  if (value.startsWith('"')) {
    if (!/^"(?:[^"\\\x00-\x1f]|\\(?:[0abtnvfre "\\/N_LP]|x[\da-fA-F]{2}|u[\da-fA-F]{4}|U[\da-fA-F]{8}))*"$/.test(value))
      return 'malformed double-quoted value';
    // YAML escapes must encode Unicode scalar values, not out-of-range points.
    for (const match of value.matchAll(/\\(?:u([\da-fA-F]{4})|U([\da-fA-F]{8}))/g)) {
      const point = Number.parseInt(match[1] || match[2], 16);
      if (point > 0x10ffff || (point >= 0xd800 && point <= 0xdfff)) return 'invalid Unicode escape';
    }
    return null;
  }
...
export function frontmatterProblems(header) {
  ...
  header.split(/\r?\n/).forEach((line, index) => {
    if (/^[ \t]*\t/.test(line)) {
      problems.push({ line: index + 1, reason: 'tab indentation is not supported' });
      return;
    }
    ...
    if (reason) problems.push({ line: index + 1, key, value, reason, repairable: key === 'description' && reason === 'plain value needs quoting' });
  });
  return problems;
}
...
export function disableLegacySkill(text, name) {
  ...
  const header = /^disable-model-invocation:/m.test(repaired)
    ? repaired.replace(/^disable-model-invocation:[^\r\n]*/m, 'disable-model-invocation: true')
