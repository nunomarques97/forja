...
import { validateWorkerAccess, restrictedSettings, restrictedMcp, workerScratch, workerAccessPrompt } from './worker-access.mjs';
    ...
    return {
      ...
      args: [
        ...
        ...(restricted ? ['--restricted', '--safe-mode', '--no-session-persistence', '--permission-prompts', 'none',
          ...
          '--allowedTools', readOnly ? 'Read,Grep,Glob' : 'Read,Grep,Glob,Write,Edit', '--disallowedTools', 'mcp__*'] : []),
        '-p',
        '--output-format',
        'stream-json',
        '--verbose',
        ...
        mcpPath,
        '--tools',
        restricted ? (readOnly ? 'Read,Grep,Glob' : 'Read,Write,Edit,Grep,Glob') : config.fullAccess === true ? 'default' : readOnly
...
export async function runProvider(provider, options) {
  ...
  try {
    result = await execute(spec.command, spec.args, {
      ...options,
      input: (options.input || '') + (provider === 'custom' ? '' : workerAccessPrompt({ scratch: scratchPath, restricted, readOnly: options.readOnly })),
      env,
