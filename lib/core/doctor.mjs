...
import { checkCommandProblem } from './check-commands.mjs';
import { validateCheckIsolation } from './check-isolation.mjs';
...
export function doctor(root, { provider = 'claude', config = {}, inspectProvider = providerInstallation } = {}) {
  ...
  try { coreBudgets(config); validateRouting(config, provider); validateFinalChecks(config); validateCheckIsolation(config); validateDelivery(config); deliveryPolicySnapshot(root, config); }
  catch { add('configuration', 'error', 'Invalid budget, routing, finalChecks, checkIsolation or delivery configuration. Check the documented schema and delivery policy file.'); return { ready: false, checks, providers: [] }; }
  add('configuration', 'ok', 'Budget, routing and finalChecks configuration is valid.');
  const hostCheck = checkCommandProblem([], config, { cwd: root });
  if (hostCheck) add('final_checks', 'error', hostCheck);
  if (config.checkIsolation) add('check_isolation', 'warning', 'Bubblewrap requested. Actual namespace support is verified at run startup before model calls; doctor only validates its configuration. Checks require Linux executables and a read-only source snapshot.');
  if (config.delivery) add('delivery', 'warning', 'Reviewer-owned delivery enabled. Requires a clean initial worktree and an unchanged reviewed snapshot; no additional model session. Push authorization and pipeline effects are separate from commit permission.');
