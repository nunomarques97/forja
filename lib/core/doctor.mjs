...
export function doctor(root, { provider = 'claude', config = {}, inspectProvider = providerInstallation } = {}) {
  ...
  try { validateRouting(config, provider); validateFinalChecks(config); }
  catch { add('configuration', 'error', 'Invalid routing or finalChecks configuration. Check the config schema and concrete command targets.'); return { ready: false, checks, providers: [] }; }
  add('configuration', 'ok', 'Routing and finalChecks configuration is structurally valid.');
