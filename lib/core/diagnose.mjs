...
import { diagnosticFilters, matchesDiagnosticFilters, DIAGNOSTIC_PHASES } from './diagnostic-filters.mjs';
...
export function readMetadata(root, name, budget, limit = 4 * 1024 * 1024) {
  let fd;
  try {
...
// Filters are validated before any file access and applied after the complete
// ledger is reconciled, so global warnings stay unfiltered and unselected
// traces are never opened or charged to the request budget.
export function diagnoseRun(root, options = {}) {
  const filters = diagnosticFilters(options);
  ...
  const selected = [...rows.values()].filter(row => matchesDiagnosticFilters(row, filters));
  const invocations = selected.sort((a, b) => a.id - b.id).map(row => {
    const file = readMetadata(root, `${prefix}/call-${row.id}-events.jsonl`, budget);
    return {
      id: row.id,
      phase: DIAGNOSTIC_PHASES.includes(row.phase) ? row.phase : null,
