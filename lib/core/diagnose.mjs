...
export function summarizeTrace(text, { provider, durationMs } = {}) {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    if (row.kind === 'end') {
      if (ended) warnings.add('invalid_record');
      ended = true;
      if (!count(row.dropped_events) || !count(row.unparsed_lines)) warnings.add('invalid_record');
    ...
    if (!entry) { entry = { type: i.type, start: null, finish: null, failed: false }; seen.set(i.id, entry); }
    if (row.type === 'item.started' && entry.start === null && entry.finish === null) {
      entry.start = time;
      r.first_tool_start_ms ??= time;
    }
    if (row.type !== 'item.completed') {
      if (row.type === 'item.started' && entry.finish !== null) warnings.add('conflicting_tool_event');
      continue;
    }
    if (!['completed', 'failed', 'declined'].includes(i.status)) { warnings.add('unknown_tool_status'); continue; }
    const failed = i.status === 'failed' || i.status === 'declined' || (Number.isSafeInteger(i.exit_code) && i.exit_code !== 0);
    if (entry.finish !== null) {
      if (entry.failed !== failed) warnings.add('conflicting_tool_event');
      continue;
    }
    entry.finish = time;
    entry.failed = failed;
