...
export function modelEvidence(root, options = {}) {
  ...
  const signals = [];
  if (warnings.size || groups.some(g => Object.entries(g.measurements).some(([k, v]) => v === null || (k.endsWith('_covered_invocations') && v < g.invocations.length)))) signals.push('measurement_gaps');
