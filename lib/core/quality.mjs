...
export function checksFor(run, task) {
  ...
  return (run.finalCheckTaskId === task.id || (run.tasks.some(t => t.status !== 'done') && run.tasks.every(t => t.id === task.id || t.status === 'done')))
...
export function reviewFocus(task, goal = '') {
  ...
  if (risk.visual) probes.push('Exercise loading, empty, error, retry and success states; verify keyboard focus and live announcements across refresh, and inspect desktop/mobile evidence.');
