...
import { captureProtectedFiles, validateProtectedFiles, assertProtectedFiles } from './protected-files.mjs';
...
export function validateState(run, root) {
  ...
  if (run.developmentReceipt !== undefined) {
    ...
    if (!receipt || receipt.version !== 1 || !task || task.status !== 'develop' ||
        ...
        !['done', 'ready_for_validation'].includes(receipt.result?.status))
...
export async function drive(
  ...
  try {
    ...
    if (invalidCheck) {
      pause(`Check preflight: ${invalidCheck} Stored checks and evidence are unchanged. Abandon this run (core abandon --why "...") and start a new run with concrete check targets.`);
      return run;
    }
    ...
    const call = async (
      ...
      if (phase === 'develop' && ['done', 'ready_for_validation'].includes(result.status)) {
    ...
    try {
      ...
      while (true) {
        ...
        if (!['validate', 'review'].includes(task.status)) {
          ...
          if (!run.developmentReceipt) task.attempts++;
          task.status = 'develop';
          save();
        ...
        if (task.status === 'validate') {
          ...
          for (let i = 0; i < requiredChecks.length; i++) {
            ...
            let protectionError;
            try { assertProtectedFiles(root, run); }
...
export async function core({ pos = [], opt = {} } = {}) {
  ...
  if (cmd === 'diagnose') {
    ...
    console.log(JSON.stringify(diagnoseRun(root), null, 2));
    return;
  }
  if (cmd === 'decide') {
