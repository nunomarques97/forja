...
import { validateFinalChecks, checksFor, reviewFocus, snapshotBoundCheck } from './quality.mjs';
...
import { RunStop, recoveryInfo, exhaustedRotations } from './recovery.mjs';
...
import { validateDelivery, deliveryPolicySnapshot, taskGranularity } from './delivery-policy.mjs';
import { freezeSpecialists, validateSpecialists, assertSpecialists, specialistGuidance } from './specialists.mjs';
import { readStopRequest, clearStopRequest, stopDue } from './stop.mjs';
import { checkCommandProblem, npmCli, WINDOWS_CHECK_GUIDANCE } from './check-commands.mjs';
...
const SUMMARY_LIMIT = 4000, FINDINGS_LIMIT = 12000, FINDING_LIMIT = 2000;
...
export const RESULT_SCHEMA = {
  ...
  properties: {
    ...
    summary: { type: 'string', maxLength: SUMMARY_LIMIT, description: `At most ${SUMMARY_LIMIT} characters; put detail in findings or project files.` },
    findings: { type: 'array', items: { type: 'string', maxLength: FINDING_LIMIT }, description: `At most ${FINDINGS_LIMIT} characters in total.` },
  },
};
...
// Windows limits a command line to 32,767 characters and git diff has no
// --pathspec-from-file: diff many paths in bounded batches. Per-file diffs of
// sorted batches concatenate to the same patch as one call.
const PATH_BATCH_CHARACTERS = 16000;
export function diffPaths(root, base, paths) {
  let patch = '', batch = [], size = 0;
  const flush = () => {
    if (batch.length) patch += git(root, ['diff', base, '--', ...batch]);
    batch = [];
    size = 0;
  };
  for (const path of paths) {
    if (batch.length && size + path.length + 3 > PATH_BATCH_CHARACTERS) flush();
    batch.push(path);
    size += path.length + 3;
  }
  flush();
...
// Changed-path lists share the 48,000-character review packet; past this bound
// the packet lists a prefix and the reviewer reads the complete list from disk.
const REVIEW_PATH_LIST_CHARACTERS = 12000;
function pathPrefix(paths, max) {
  const prefix = [];
  let size = 2;
  for (const path of paths) {
    size += JSON.stringify(path).length + 1;
    if (size > max) break;
    prefix.push(path);
  }
  return prefix;
...
const interruptedStop = (what) => new RunStop('interrupted',
  `${what} interrupted by the operator; no result was recorded and no implementation attempt was spent. Work on disk is preserved; inspect core status, then core resume.`);
...
function checkCommand(check) {
  ...
  if (check.command === 'npm' && process.platform === 'win32')
    return {
      ...
      args: [
        npmCli() || join(dirname(process.execPath), 'node_modules/npm/bin/npm-cli.js'),
        ...check.args,
      ],
    };
  return check;
...
export function validatePlan(plan, root, { strict = true, links = true } = {}) {
  ...
  for (const t of plan.tasks) {
    ...
    for (const f of t.files) inside(root, f, { links });
...
export function validateResult(r) {
  ...
  const problem =
    !r ? 'no result'
    : !['done', 'ready_for_validation', 'blocked', 'approve', 'reject', 'checkpoint'].includes(r.status) ? 'unknown status'
    : typeof r.summary !== 'string' ? 'summary is not a string'
    : r.summary.length > SUMMARY_LIMIT ? `summary has ${r.summary.length} characters (limit ${SUMMARY_LIMIT})`
    : !Array.isArray(r.findings) || r.findings.some((s) => typeof s !== 'string') ? 'findings is not a list of strings'
    : JSON.stringify(r.findings).length > FINDINGS_LIMIT ? `findings have ${JSON.stringify(r.findings).length} characters (limit ${FINDINGS_LIMIT})`
    : null;
  if (problem) throw new Error(`Invalid/oversize worker result: ${problem}.`);
  return r;
}

// A structurally valid result whose text only exceeds the stored limits is
// shortened instead of discarding completed work; the provider output file
// keeps the full text.
export function boundResult(r, evidence) {
  if (!r || typeof r !== 'object') return r;
  const note = `[truncated; full text in ${evidence}]`;
  const bounded = { ...r };
  if (typeof r.summary === 'string' && r.summary.length > SUMMARY_LIMIT)
    bounded.summary = `${r.summary.slice(0, SUMMARY_LIMIT - note.length - 2)}… ${note}`;
  if (Array.isArray(r.findings) && r.findings.every((s) => typeof s === 'string') && JSON.stringify(r.findings).length > FINDINGS_LIMIT) {
    const kept = [];
    for (const finding of r.findings.map((s) => s.length > FINDING_LIMIT ? `${s.slice(0, FINDING_LIMIT - note.length - 2)}… ${note}` : s)) {
      if (JSON.stringify([...kept, finding, note]).length > FINDINGS_LIMIT) break;
      kept.push(finding);
    }
    bounded.findings = [...kept, `${r.findings.length - kept.length} more findings ${note}`];
  }
  return bounded;
}
export function validateState(run, root) {
  ...
  if (run.tasks.length)
    validatePlan({ tasks: run.tasks, decisions: run.decisions }, root, { strict: false, links: false });
  for (const task of run.tasks) {
    ...
    // Lexical only: a link a worker left behind must not make status, resume
    // and abandon unreadable. Reads and writes still refuse links outside.
    for (const path of Object.keys(task.before || {})) inside(root, path, { links: false });
  }
  ...
  const commitId = value => /^[a-f0-9]{40,64}$/.test(value);
  if (run.taskCommits !== undefined && (!taskGranularity(run) || !Array.isArray(run.taskCommits) ||
      run.taskCommits.some((c, i) => !c || !run.tasks.some(t => t.id === c.task) || !commitId(c.commit) ||
        c.parent !== (i ? run.taskCommits[i - 1].commit : run.base) || typeof c.receipt !== 'string')))
    throw new Error('Invalid task commit chain in core state.');
  if (run.taskDelivery !== undefined && (!taskGranularity(run) || !run.tasks.some(t => t.id === run.taskDelivery?.task) ||
      !/^[A-Za-z][A-Za-z0-9_-]{0,40}-\d{1,5}$/.test(run.taskDelivery.key || '')))
    throw new Error('Invalid pending task delivery in core state.');
  return run;
}
...
export function createRun(
  ...
  const hostCheck = checkCommandProblem(run.tasks, config, { cwd: root });
  if (hostCheck) throw new Error(hostCheck);
...
async function driveWork(
  ...
  // Stop requests older than this controller session belong to a stopped or
  // ended session; starting or resuming clears them.
  ...
  try {
    ...
    const invalidCheck = planCheckProblem(run.tasks, run.config.finalChecks) ||
      checkCommandProblem(run.tasks, run.config, { cwd: root });
    if (invalidCheck) {
    ...
    const staleStop = readStopRequest(root, run.run_id);
    if (!staleStop || staleStop.requested_at < sessionStart) clearStopRequest(root);
    run.status = 'running';
    delete run.stopCode;
    delete run.operatorStop;
    save();
    // Honour an operator stop request at an invocation boundary: the previous
    // result is recorded and no attempt, rotation or session is consumed.
    const operatorStop = (nextTask) => {
      const request = readStopRequest(root, run.run_id);
      if (!stopDue(request, nextTask?.id)) return false;
      run.operatorStop = { mode: request.mode, task: request.task, requested_at: request.requested_at, stopped_at: now(), next_task: nextTask?.id ?? null };
      pause(`Operator stop (${request.mode === 'task' ? `after task ${request.task ?? 'planning'}` : 'next invocation boundary'}): no worker was interrupted. Resume with core resume.`, 'operator_stop');
      clearStopRequest(root);
      return true;
    };
    // Task granularity: missing or rejected delivery approval stops further
    // tasks; earlier commits and working files stay as they are.
    const blockTaskDelivery = (task, reason) => {
      task.status = 'blocked';
      task.feedback = { status: 'blocked', summary: `${reason} Earlier task commits and working files are preserved. Inspect, then core retry --task ${task.id} --validate-only --why "..." for fresh checks and review of this snapshot.`, findings: [] };
      save();
      throw new RunStop('inspect', `${task.id}: ${task.feedback.summary}`);
    };
    const finishTaskDelivery = async (task) => {
      if (run.taskDelivery?.task === task.id && run.taskDelivery.status === 'no_changes') {
        task.delivery = { status: 'no_changes' };
        delete run.taskDelivery;
        return;
      }
      const { commitTaskDelivery } = await import('./delivery.mjs');
      try {
        task.delivery = { status: 'committed', commit: commitTaskDelivery(root, run, task, save) };
      } catch (error) {
        // A created but uninstalled commit is reconciled on resume, never recreated.
        if (run.taskDelivery?.status === 'prepared') throw new RunStop('inspect', `${task.id}: ${error.message} Resume retries installing the same commit.`);
        blockTaskDelivery(task, `Task delivery blocked: ${error.message}`);
      }
    };
    ...
    const call = async (
      ...
      const framing =
        ...
        (changes?.delivery?.status === 'ready' && changes.delivery.granularity === 'task' ? ' As the reviewer of this task, also own approval of its commit in this same session. Inspect the referenced delivery manifest and patch (the task delta against the last approved task commit) for private/unrelated content; the manifest lists earlier approved commits and, for a push run, the whole outgoing chain published after the run completes. Return delivery.status approve or reject, the exact delivery.tree, a concise public-safe commit message describing the change of this task and a short reason. Approval covers this snapshot only and never grants production permission. Do not execute Git commands; the controller performs approved operations.' : '') +
        (changes?.delivery?.status === 'ready' && changes.delivery.granularity !== 'task' ? ' As the final reviewer, also own delivery approval in this same session. Inspect the referenced delivery manifest and patch for private/unrelated content, prior approved work as needed, and the declared destination/pipeline effects. Return delivery.status approve or reject, the exact delivery.tree, a concise public-safe commit message and a short reason. Approval covers this snapshot only and never grants production permission. Do not execute Git commands; the controller enforces permissions and performs approved operations. A missing production grant alone does not invalidate code or public readiness.' : '') +
      ...
      const creative = phase === 'plan'
        ...
        : '';
      const hostChecks = phase === 'plan' && process.platform === 'win32' && !run.config.checkIsolation ? WINDOWS_CHECK_GUIDANCE : '';
      const prompt = `${RULES}\n${tech}\n${instruction}${creative}${hostChecks}${quality}${framing}${ctx.text}`;
      ...
      const record = {
        ...
        result: out.interrupted ? 'interrupted' : out.code === 0 && !out.error && !out.rate_limited && !out.timedOut && !out.overflow ? 'returned' : 'error',
      ...
      if (out.interrupted) {
        // An operator interruption is not an implementation failure.
        if (phase === 'develop') { task.attempts = Math.max(0, task.attempts - 1); task.status = 'todo'; }
        throw interruptedStop(`Provider ${phase} call-${id}`);
      }
      if (forcedCheckpoint) {
        out.result = {
          status: 'checkpoint',
      ...
      const result = phase === 'plan' ? out.result : validateResult(boundResult(out.result, `call-${id}-stream.json`));
    ...
    try {
      ...
      lock.child(null);
      if (!run.tasks.length) {
        if (operatorStop(null)) return run;
        const plan = await call('plan');
        // Apply a plan atomically: record its technology before tasks exist, so a
        // rejected assessment can never leave runnable tasks without a cost gate.
        const tasks = validatePlan(plan, root);
        const hostCheck = checkCommandProblem(tasks, run.config, { cwd: root });
        if (hostCheck) throw new Error(`Plan refused before any task: ${hostCheck} Resume to plan again.`);
        addTechnology(run, plan.technology);
        run.tasks = tasks;
      ...
      if (await sponsorGate()) return run;
      // An approved task commit interrupted before its task was recorded as
      // done is finished from its receipt, without another review session.
      const approvedDelivery = taskGranularity(run) && ['approved', 'prepared'].includes(run.taskDelivery?.status) &&
        run.tasks.find(t => t.id === run.taskDelivery.task && t.status === 'review' && t.review?.status === 'approve' && t.validated_tree === treeHash(root));
      if (approvedDelivery) {
        await finishTaskDelivery(approvedDelivery);
        Object.assign(approvedDelivery, { status: 'done', reviewed_tree: approvedDelivery.validated_tree, completed_at: now(), feedback: null });
        save();
      }
      while (true) {
        assertRunInputs(root, run);
        if (run.tasks.every((t) => t.status === 'done')) {
          ...
          for (const previous of run.tasks) {
            ...
            for (let i = 0; i < finalChecks.length; i++) {
              ...
              // Validated at the task's own snapshot; later accepted work
              // necessarily changes the diff it asserts is empty.
              if (previous.checks.includes(check) && snapshotBoundCheck(check)) {
                (previous.finalValidation ||= []).push({ command: check.command, args: check.args, skipped: 'snapshot_bound', passed: true });
                save();
                continue;
              }
              ...
              lock.child(null);
              if (checked.interrupted) throw interruptedStop(`Final check ${i + 1} of ${previous.id}`);
        ...
        if (operatorStop(task)) return run;
        if (!task.before) {
        ...
        if (!['validate', 'review'].includes(task.status)) {
          ...
          task.status = 'develop';
          save();
          const launchedBefore = run.invocations;
          let result;
          try {
            result = await call('develop', task, task.feedback);
          } catch (error) {
            // A refusal before launch (packet size, knowledge, protected files,
            // frozen methods) spends no implementation attempt.
            if (run.invocations === launchedBefore && !run.developmentReceipt) {
          ...
          if (result.status === 'checkpoint') {
            ...
            if (task.stalled_rotations >= STALLED_ROTATION_LIMIT)
              throw new RunStop('no_progress_between_rotations', `${task.id}: ${task.stalled_rotations} consecutive context-limit sessions changed no source and no progress notes. Split the task into smaller read scopes instead of spending another rotation.${exhaustedRotations(run, task)}`);
            if (task.context_limit_streak >= CONTEXT_LIMIT_STREAK_LIMIT)
              throw new RunStop('repeated_context_limit', `${task.id}: ${task.context_limit_streak} consecutive sessions reached the context limit without an intentional handoff. Source edits and notes alone do not justify another automatic rotation. Inspect preserved work and narrow the task or explicitly revise its context budget.${exhaustedRotations(run, task)}`);
            continue;
          }
        ...
        if (task.status === 'validate') {
          ...
          for (let i = 0; i < requiredChecks.length; i++) {
            ...
            lock.child(null);
            if (result.interrupted) throw interruptedStop(`Check ${i + 1} of ${task.id}`);
        ...
        const patch = diffPaths(root, run.base, changed);
        ...
        const newFiles = changed.filter((p) => !task.before[p]);
        const listsFit = JSON.stringify(changed).length + JSON.stringify(newFiles).length <= REVIEW_PATH_LIST_CHARACTERS;
        const fileList = join(dir, `${task.id}-changed-files.json`);
        if (!listsFit) write(inside(root, fileList), { files: changed, new_files: newFiles });
        let delivery = null;
        if (run.config.delivery) {
          const { prepareDelivery, prepareTaskDelivery } = await import('./delivery.mjs');
          delivery = taskGranularity(run) ? prepareTaskDelivery(root, run, task) : prepareDelivery(root, run, task);
        }
        save();
        // A task commit that cannot be prepared blocks before paying for review.
        if (taskGranularity(run) && delivery?.status === 'blocked') blockTaskDelivery(task, `Task delivery blocked before review: ${delivery.reason}`);
        if (operatorStop(task)) return run;
        const review = await call(
          ...
          task.result_path
            ...
            : task.feedback,
          {
            files: listsFit ? changed : pathPrefix(changed, REVIEW_PATH_LIST_CHARACTERS / 2),
            patch: join(dir, `${task.id}-change.patch`),
            risk: task.risk,
            validation: task.validation,
            new_files: listsFit ? newFiles : pathPrefix(newFiles, REVIEW_PATH_LIST_CHARACTERS / 2),
            ...(listsFit ? {} : { files_total: changed.length, new_files_total: newFiles.length, file_list: fileList,
              file_list_note: 'The path lists above are a prefix; file_list holds every changed and new path.' }),
            ...(delivery ? { delivery } : {}),
        ...
        if (delivery?.status === 'ready') {
          const { recordDeliveryReview, recordTaskDeliveryReview } = await import('./delivery.mjs');
          (taskGranularity(run) ? recordTaskDeliveryReview : recordDeliveryReview)(root, run, task, review);
        }
...
export function recoverRun(
  root,
  { action = 'resume', taskId, reason, limits = {}, validateOnly = false, reopen = false } = {},
) {
  const lock = lockProject(root);
  try {
    ...
    if (action === 'retry') {
      ...
      if (!task) throw new Error('Retry requires a --task ID of this run.');
      if (reopen !== (task.status === 'done'))
        throw new Error(reopen
          ? 'Reopen applies only to an approved (done) task; use retry without --reopen.'
          : 'Retry requires an unfinished --task ID; reopen an approved task with --reopen when a defect is found after approval.');
      if (reopen && validateOnly)
        throw new Error('A reopened task needs new implementation; --validate-only does not apply.');
      if (['approved', 'prepared'].includes(run.taskDelivery?.status))
        throw new Error(`An approved commit for ${run.taskDelivery.task} is pending installation; resume to finish it before a retry.`);
      if (
      ...
      if (reopen) {
        // An approval that fails in real use: the task is implemented, checked
        // and reviewed again against the current tree; earlier evidence stays.
        (task.reopened ||= []).push({ at: now(), reason, review: task.review?.status || null });
        for (const key of ['validated_tree', 'reviewed_tree', 'completed_at', 'before']) delete task[key];
      }
      task.feedback = {
        status: reopen ? 'reject' : 'checkpoint',
        summary: reopen ? `Reopened after approval: ${reason}` : reason,
        findings: reopen ? [] : task.feedback?.findings || [],
      };
    }
    if (action === 'abandon') {
      run.status = 'failed';
...
export async function core({ pos = [], opt = {} } = {}) {
      ...
      JSON.stringify(
        {
          ...
          stop_request: readStopRequest(root, run.run_id),
          delivery: run.delivery || null,
          ...(taskGranularity(run) ? { delivery_granularity: 'task', task_commits: (run.taskCommits || []).map(c => ({ task: c.task, commit: c.commit })),
            pending_task_delivery: run.taskDelivery ? { task: run.taskDelivery.task, status: run.taskDelivery.status, reason: run.taskDelivery.reason } : null } : {}),
  ...
  if (cmd === 'stop') {
    const { requestStop } = await import('./stop.mjs');
    const { coreAlive } = await import('./observe.mjs');
    const { run, request, changed } = requestStop(root, { afterTask: !!opt['after-task'], controllerAlive: () => coreAlive(root) });
    const boundary = request.mode === 'task'
      ? `after task ${request.task ?? 'planning'} finishes its checks, review and repairs within budget`
      : 'at the next invocation boundary';
    console.log(JSON.stringify({ run: run.run_id, stop_request: request, changed,
      message: `${changed ? 'Stop requested' : 'Stop already requested'}: the controller stops ${boundary}, without interrupting a live worker. Resume later with core resume.` }, null, 2));
    return;
  }
  const limitFlags = [
  ...
  // The argument parser stores a flag without a value as true.
  for (const [flag] of limitFlags)
    if (opt[flag] === true) throw new Error(`--${flag} needs an integer value.`);
  ...
  if (cmd === 'start') {
    ...
    for (const [flag, , key] of limitFlags)
      if (opt[flag] !== undefined) config[key] = key === 'maxCloudSessions' ? budget(opt[flag], undefined, 0, 200) : opt[flag];
    ...
    throw new Error(
      'Use forja core init|start|resume|retry|abandon|stop|status|usage|evidence|evaluation-plan|benchmark|context|decide|deliver',
    );
  const { upsertProject } = await import('../projects.mjs');
  upsertProject({ path: root });
