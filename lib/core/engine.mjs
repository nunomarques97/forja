...
export async function drive(
  ...
  try {
    ...
    const call = async (
      ...
      const instruction =
        phase === 'plan'
          ? 'Inspect relevant code and project instructions. Return a plan with 1–30 cohesive tasks, concise objective criteria and executable checks. No minimum task count: prefer one task for a cohesive feature touching the same files. Each after array contains prerequisite task IDs from this plan only, or [] when none; it never contains instructions. Task checks must be achievable by that task; caller final_checks run at integration, so do not assign whole-goal checks to incomplete intermediate work. Decisions contain consequential choices only, without restating the goal or rules. Risks contain only applicable labels: security, visual, architecture. Include browser/screenshot acceptance for UI changes, security for trust boundaries and architecture for structural changes. Do not edit project code. Do not invoke other agents.'
          : phase === 'develop'
