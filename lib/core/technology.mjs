...
export function technologyGuidance(phase) {
  if (phase === 'plan') return ' Assess technology only for a new capability, material dependency or consequential unresolved choice; routine work using an accepted stack returns technology=[]. Do not reassess the already configured execution provider or approved project costs. Within this planning session compare 2-3 viable options, including the existing stack when viable, against project constraints; cite inspected local evidence or current primary sources for costs and tradeoffs. Use at most three primary references per choice, not an open-ended survey; if evidence or pricing is uncertain label cost unknown. Any relevant paid or unknown-cost option requires Sponsor choice, even if you recommend a free option. Do not purchase, subscribe, provision or use a paid service. A free tier counts as free only with evidence it covers the stated requirements. Do not schedule a separate research agent/task merely to fill this field.';
  return ' Honor recorded technology choices. If a material new technology choice or paid/uncertain cost is discovered, stop before adoption or paid use and return blocked with technology alternatives/evidence; never infer Sponsor approval. Otherwise return technology=[]. Reviewer must reject unauthorized technology/cost changes. Sponsor selection authorizes the recorded implementation choice, never a purchase, subscription or credential access.';
}
