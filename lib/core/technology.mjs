...
export function validateTechnology(items = [], maxItems = 3) {
  ...
  for (const d of items) {
    ...
    // Case-only duplicates would make the later addition fail after a plan was applied.
    const capability = d.capability.toLowerCase();
    if (capabilities.has(capability)) throw Error('Duplicate technology capability; assess each capability once.');
