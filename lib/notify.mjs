...
// The topic comes only from FORJA_NTFY_TOPIC: there is no default, so a fresh
// clone never posts to somebody else's topic. Unset → every notify() is skipped
// (logged as skipped: 'no-topic') and nothing leaves the machine.
...
const logPath = join(process.env.FORJA_DATA_DIR || join(repoRoot, 'data'), 'notify.log');

export function topic() {
  return (process.env.FORJA_NTFY_TOPIC || '').trim() || null;
}
...
export async function notify(message, opts = {}) {
  ...
  if (!topic()) { logLine({ ts: new Date().toISOString(), message, ok: false, skipped: 'no-topic' }); return { ok: false, skipped: 'no-topic' }; }
  if (dedup && recentlySent(message)) { logLine({ ts: new Date().toISOString(), message, ok: true, skipped: 'dedup' }); return { ok: true, skipped: 'dedup' }; }
  const url = `${process.env.FORJA_NTFY_SERVER || DEFAULT_SERVER}/${topic()}`;
