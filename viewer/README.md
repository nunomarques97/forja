...
Each readable Core project in `GET /api/core` includes `validation_summary: { passed, failed, unknown, tasks_without_records }`, four non-negative integers projected from the latest validation records in the current run. For every task whose `validation` is a non-empty array, each entry adds to `passed` when `passed === true`, to `failed` when `passed === false`, and to `unknown` otherwise (null entries, `{}`, strings such as `'true'`, `null`). A task whose `validation` is missing, empty or not an array adds one to `tasks_without_records`: no records is not a pass. A run with no tasks returns all zeros. Only these counts are exposed; validation commands, logs, paths and output never cross the API. The counts describe the latest checks of the current run, not a history and not release approval or independent review. The field is present only on detailed observations (`/api/core`), never on error projects. Clients must treat a missing summary from an older server, or any summary that is not an object with all four fields as non-negative safe integers, as unavailable ("Validation evidence unavailable") rather than as zero failures. Malformed validation entries no longer turn the project into an error; task rows count `checks_total` from array lengths only and `checks_passed` from `passed === true` only.
...
Core detail responses may include `recovery`: a closed diagnostic code, public-safe title and guidance, used/limit counters for total and cloud sessions, per-call minutes, and the configured context threshold with its measurement limits. It is absent or null for active/finished runs and pending Sponsor choices. Raw provider errors, local paths and call arguments are never copied into this projection. Missing fields remain compatible with older servers. Recovery stays in the CLI; these diagnostics do not retry calls or change budgets. Usage rows expose observed timeout/context/quota flags and configured time/context limits when available. Context stopping is measured for Claude requests only; Codex/custom have no comparable live measurement.
