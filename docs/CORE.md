...
For asynchronous UI, verify transitions as well as final screenshots: overlapping success/failure responses, invalidation while paging, refresh from an empty result, retry after failed refresh, and keyboard focus/live announcements during updates. Trace criteria to executed evidence and inspect cases omitted by the supplied checks. User-owned final checks are additional requirements; never weaken or rewrite their oracle to pass.
