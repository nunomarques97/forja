...
O doctor aplica aos budgets de execução as mesmas regras do arranque: `maxSessions` 1–200 (default 30), `maxAttempts` 1–5 (2), `maxMinutes` 1–180 (30), `maxRotations` 0–5 (2) e `maxContextTokens` 1.000–1.000.000 (120.000). Aceita inteiros e strings só com dígitos decimais sem alterar a configuração; booleanos, listas, hexadecimal, expoentes, sinais, decimais e espaços são recusados, e uma flag de limite sem valor (por exemplo `--max-sessions` sozinha) também; um valor inválido devolve um erro de configuração antes de inspecionar executores. `maxCloudSessions` mantém a validação estrita de routing: número inteiro 0–200.
...
## Migrar um projeto legado

Para primeiro os executores do projeto, preservando alterações por concluir. Corre `node <forja>/bin/forja.mjs core init` na raiz do projeto. O init substitui o antigo bloco gerido `forja:begin` por orientação Core, consolida-o com um bloco Core já existente e mantém todas as instruções fora desses blocos. Desativa a invocação automática das skills legadas reconhecidas com `disable-model-invocation: true`, preservando o corpo para uso legado explícito. Torna também manuais as cópias dos métodos Core no projeto (`forja-core-*`): os workers recebem cópias congeladas através de `specialist_context`, nunca por descoberta nativa de skills. Os agentes da crew legada em `.claude/agents/` (os nove papéis da crew e os nomes antigos, apenas ficheiros que mencionam FORJA) são movidos, nunca apagados, para `docs/forja/legacy-agents/`, para que a ferramenta Agent deixe de os escolher pela descrição; a saída lista cada movimento em `legacy_agents`, e devolver os ficheiros ao sítio repõe a crew legada. Enquanto `docs/forja/RUN.json` não registar um estado terminal (`finished`, `failed` ou `abandoned`), ou não puder ser lido, os agentes ficam no sítio e a saída indica porquê. Um ficheiro de arquivo com conteúdo diferente para o init antes de qualquer escrita. O arquivo fica excluído da descoberta de conhecimento e do mapa do repositório. O init não copia uma crew, não altera perfis de modelos, não fecha runs, não lança workers nem muda permissões de publicação.

`forja bootstrap <repo>` faz a mesma preparação Core que `core init` e regista o projeto; `--dry-run` mostra as alterações sem escrever. A instalação da crew legada (agentes, skills, hooks e ficheiros de run legados) só corre com `--legacy` explícito; nesse modo, `--keep-legacy` mantém os ficheiros com nomes antigos da crew que seriam removidos.

O Claude Code ignora um cabeçalho de skill que não seja YAML válido, incluindo a flag, por isso o init valida o cabeçalho de cada skill legada depois da alteração. Um valor simples que o YAML recusaria, como uma descrição com `: `, é posto entre aspas; qualquer outro cabeçalho inválido para o init antes de escrever. Correr o init outra vez num projeto já migrado repara esses cabeçalhos e, de resto, é idempotente. Marcadores malformados, duplicados ou sobrepostos e ficheiros de skill legada não reconhecidos falham antes de escrever. Os destinos de skills existentes têm proteção de contenção e reposição. Revê o diff resultante; reinicia a conversa para que um método Lead já carregado não continue a conduzi-la. As skills legadas continuam legíveis como ficheiros ou invocáveis explicitamente pelo utilizador; ver [controlo de invocação de skills do Claude](https://code.claude.com/docs/en/skills#control-who-invokes-a-skill).

Se `docs/forja/RUN.json` ainda registar um run em curso ou bloqueado, fecha-o explicitamente depois de confirmar que os executores pararam. Usa `run fail --why "Stopped for Core migration; unfinished work preserved"` num run interrompido; nunca marques trabalho incompleto como concluído. A migração por si só deixa esse estado intacto, e o Core continua a recusar um run legado ativo. Indica o perfil `--config` escolhido no novo run Core e inspeciona as alterações pendentes antes de usar `--allow-dirty`.

A orientação é portável: resolve a instalação a partir de quem chama, de `FORJA_ROOT` ou de um caminho de hook FORJA existente. Não faças commit de um caminho local da pasta pessoal. Referências históricas a papéis da crew não autorizam delegação manual num run Core; mapeia as restrições de produto aplicáveis para as fases do controlador.

A migração só arquiva definições reconhecidas da crew FORJA, não um agente do utilizador que apenas mencione FORJA. Um estado ausente ou desconhecido num ficheiro de run legado existente conta como ativo; só estados terminais explícitos permitem arquivar. Colisões no arquivo preservam o ficheiro do outro escritor. O verificador de YAML suporta um subconjunto conservador de cabeçalhos, repara descrições simples e recusa estruturas não suportadas em vez de lhes mudar o tipo.
...
A notificação usa `FORJA_NTFY_TOPIC` ou `notify-config.json` na pasta de dados da instalação FORJA (`data/`, ou `FORJA_DATA_DIR` quando definido), conforme [RELEASE.md](RELEASE.md). O servidor ntfy por omissão é `https://ntfy.sh`; `FORJA_NTFY_SERVER` indica outro. Cada envio tentado, com sucesso ou falha, fica registado em `notify.log` nessa mesma pasta de dados. Envia apenas estado e, quando disponível, link sanitizado para `/core`; nunca inclui alternativas, conteúdo de projeto ou token de acesso. A tentativa fica registada antes do envio, com teto de cinco segundos, e não se repete para o mesmo conjunto pendente. Transporte indisponível, configuração ausente ou interrupção durante o envio podem impedir a entrega; o bloqueio continua visível no painel/CLI. Reiniciar não garante reenviar uma tentativa interrompida.
...
Motivos de `recovery.code`: `context` e `rotations` (limite de contexto ou de continuações; aumenta `--max-rotations`), `no_progress_between_rotations` e `repeated_context_limit` (ver [paragens repetidas por contexto](#paragens-repetidas-por-contexto) e a deteção de livelock abaixo), `attempts`, `sessions`, `cloud_sessions` e `timeout` (budget ou tempo esgotado), `output` (limite de saída), `provider` e `provider_limit` (falha, autenticação ou quota do provider; sem retry nem troca automática de provider), `check_targets` (check com marcador por resolver), `interrupted` (controlador ausente ou Ctrl+C durante um check ou uma sessão) e `inspect` (sem causa estruturada). A orientação completa de cada motivo aparece em `recovery.guidance`; os comandos estão em [Retomar e resolver bloqueios](#retomar-e-resolver-bloqueios).
...
Cada tarefa precisa de critérios e checks (`command` e array `args`, sem concatenação de shell). Node executa-os e grava saída, duração e código. Uma falha volta ao developer dentro do orçamento, sem comprar uma revisão. O revisor inspeciona código/evidência sem editar. No fim, checks anteriores cujo código mudou são repetidos; regressões reabrem tarefas sob os mesmos limites. Checks de tarefa do tipo `git diff --exit-code` ou `--quiet` provam o estado só nessa tarefa: a regressão final regista-os como `skipped: snapshot_bound` em vez de os repetir, e o plano recebe o aviso `snapshot_check`.
...
Ctrl+C durante um subprocesso termina-o, preserva o trabalho e pausa o run com `interrupted`. Um check interrompido não fica registado nem conta como falha, e a retoma volta a correr os checks. Uma sessão de developer interrompida devolve a tentativa; a sessão continua contada. Uma morte abrupta pode deixar um worker vivo, verificado antes da retoma:
...
`retry` preserva alterações/contadores; recusa tarefas concluídas e limites esgotados sem aumento explícito. Quando uma tarefa aprovada se revela defeituosa mais tarde (por exemplo em hardware real), `core retry --task T1 --reopen --why "..."` reabre-a num run ainda não terminado: o motivo chega ao developer como correção, a tarefa volta a passar por implementação, checks e revisão independente, a reabertura fica em `reopened` da tarefa e em `recovery.jsonl`, e a tentativa conta para `--max-attempts`. Tarefas bloqueadas depois dela continuam a precisar do seu próprio `retry`. `--validate-only` começa nos checks, sem nova implementação, e continua a exigir revisão independente. `abandon` termina como `failed` e permite novo objetivo, mantendo ficheiros/evidência. Código alterado entre checks e revisão invalida a evidência e força nova validação. Desde 0.8.2, alterações após a última aprovação e antes de o run terminar também exigem novos checks e revisão; passar a regressão final não renova uma aprovação sobre outra versão. A retoma sem alterações não chama outro revisor, e runs já terminados não são reabertos automaticamente.
...
`runner`, `run`, `task`, `status`, `resume`, `bootstrap --legacy` e `forjalvl` continuam no fluxo antigo; `bootstrap` sem `--legacy` prepara o projeto para Core, como `core init`. Viewer e guarda reconhecem ambos os fluxos. Para Core, usa os comandos deste manual. Os dois fluxos recusam novos runs ativos sobrepostos. Termina/encerra explicitamente o run legado antes de usar `start`; `docs/forja/RUN.json` não é reescrito. A compatibilidade preserva projetos existentes; o Core é a metodologia recomendada para novos runs.
