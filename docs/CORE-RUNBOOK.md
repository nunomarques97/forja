...
Stop the project's executors first, preserving unfinished changes. Run `node <forja>/bin/forja.mjs core init` from the project root. Init replaces the old `forja:begin` managed block with Core guidance, consolidates it with an existing Core block, and retains all instructions outside those blocks. It disables automatic invocation of recognized legacy skills using `disable-model-invocation: true`, preserving their bodies for explicit legacy use. It also makes project copies of the Core methods (`forja-core-*`) manual-only: workers receive frozen copies through `specialist_context`, never by native skill discovery. Legacy crew agents in `.claude/agents/` (the nine crew roles and their older names, only files that mention FORJA) are moved, never deleted, to `docs/forja/legacy-agents/`, so the Agent tool can no longer select them by description; the output lists each move under `legacy_agents`, and moving the files back restores the legacy crew. While `docs/forja/RUN.json` records a running or blocked legacy run (or cannot be read), the agents stay in place and the output says why. An archive file with different content stops init before it writes anything. The archive is excluded from knowledge discovery and the repository map. It does not copy a crew, alter model profiles, close runs, launch workers or change publication permission.

`forja bootstrap <repo>` now performs the same Core preparation as `core init` and registers the project; `--dry-run` reports the changes without writing. The legacy crew install (agents, skills, hooks and legacy run files) runs only with an explicit `--legacy`.
